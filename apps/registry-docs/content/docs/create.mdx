...
```bash
bash -i <(curl https://registry.514.ai/install.sh) [connector-name] [version] [author] [language]
```

**Example:**
```bash
bash -i <(curl https://registry.514.ai/install.sh) ads-b-dot-lol v2 fiveonefour typescript
```
...
2. **Type-specific specs**: Include the appropriate specification:
   - [API Connector Spec](/docs/connectors/specifications) for general REST/GraphQL APIs
   - [SaaS Connector Spec](/docs/connectors/specifications) for third-party SaaS platforms
   - [Database Connector Spec](/docs/connectors/specifications) for SQL databases  
   - [Blob Storage Connector Spec](/docs/connectors/specifications) for cloud storage
...
```bash
bash -i <(curl https://registry.514.ai/install.sh) [pipeline-name] [version] [author] [language]
```

**Example:**
```bash
bash -i <(curl https://registry.514.ai/install.sh) google-analytics-to-clickhouse v1 fiveonefour typescript
```

This creates a new directory with your pipeline scaffold in the chosen language.

## 3. Define your pipeline

A pipeline consists of:

- **Source connector**: Where data is extracted from
- **Destination connector**: Where data is loaded to
- **Transformation logic**: How data is processed between source and destination
...
1. **Source and destination documentation**: Share the docs for both systems
2. **Pipeline specifications**: Include the pipeline specification
3. **Example pipelines**: Reference existing pipelines in the registry for patterns
   - [Google Analytics to ClickHouse](/pipeline-registry/google-analytics-to-clickhouse/v1/fiveonefour/typescript/README.md)
   - [HubSpot to ClickHouse](/pipeline-registry/hubspot-to-clickhouse/v3/fiveonefour/typescript/README.md)

Your LLM can then help generate the pipeline implementation following the established patterns and specifications.
