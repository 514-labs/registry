...
```bash
bash -i <(curl https://pipelines.514.ai/install.sh) [pipeline-name] [version] [author] [language]
```

**Example:**
```bash
bash -i <(curl https://pipelines.514.ai/install.sh) google-analytics-to-clickhouse v1 fiveonefour typescript
```
