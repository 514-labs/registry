...
export async function POST(req: NextRequest) {
  ...
  const title = `Pipeline request: ${name} (${identifier})`;
  ...
  bodyLines.push(description);
  // Embed a parseable JSON block so we can programmatically list requested pipelines
  try {
    ...
    bodyLines.push("");
    bodyLines.push("<!-- pipeline-request:start -->");
    bodyLines.push("```json");
    bodyLines.push(JSON.stringify(payloadForJson, null, 2));
    bodyLines.push("```");
    bodyLines.push("<!-- pipeline-request:end -->");
  } catch {}
