export const metadata = {
  ...
  description: "A transport-agnostic specification for building robust, production-ready blob/object storage connectors (e.g., S3, Azure Blob Storage, GCS). This release is scoped to the Extract (read-only) profile."
}
...
Note: There is no bulk `getObjects` method. To retrieve many objects, use `listObjects` with a `prefix` (and optional `delimiter`) to enumerate keys, then call `getObject` per item. Provider backends generally do not support server-side globs; patterns like `foo*` are expressed as `prefix: "foo"` with optional client-side filtering. This mirrors the API Connector Specification approach, which favors `paginate` + `batch` composition over bulk-get methods.

Bulk fetch pattern (conceptual):
