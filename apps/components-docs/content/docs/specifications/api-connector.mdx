export const metadata = {
  ...
  description: "Complete specification for implementing robust, production-ready API connectors. This release is scoped to Analytical (Extract, read-only) use cases."
}
...
- **Read-only posture**: Unsafe methods (POST/PUT/PATCH/DELETE) are Full profile only and out of scope for this release.
- **Iterator-first**: Long-running scans via `paginate` with resume; large `pageSize` within service limits.
- **Throughput-oriented**: Bounded concurrency, backpressure-aware streaming reads, cost-aware request batching.
- **Eventual consistency tolerant**: Handle duplicates/reordering; dedupe on stable identifiers (e.g., `id`, `etag`, `updated_at`).
- **Checkpointable/resumable**: Support since-cursors (e.g., `updated_since`, ETag/If-Modified-Since), continuation tokens, and link-based pagination.
- **Observability for analytics**: Emphasize bytes/rows read, throughput, page amplification, checkpoint lag, retry counts.

Recommended defaults:
- Prefer the service-maximum page size when safe; adaptively tune when erroring/throttled.
- Use a global concurrency limit with separate caps for downloads/streaming where applicable.
- Apply exponential backoff with jitter; respect Retry-After; enforce per-operation retry budgets.
...
- (Full profile only) **upload(options)** / **download(options)**  
  Large payload transfer helpers. For this release, focus on read/download; uploads are Full profile only.

#### Bulk Retrieval Pattern (Analytical)

There is no `getMany`/bulk-get method. Compose `paginate` to enumerate resources, then `batch` GET requests within concurrency and rate limits.

```text
pages = paginate({ path: "/items", pageSize: 1000 })
for page in pages:
  ops = page.items.map(item => ({ method: "GET", path: `/items/${item.id}` }))
  results = batch(ops)
  process(results)
```
...
- Performance benchmarks (throughput, page amplification, end-to-end time for large scans)
- Checkpoint/resume tests for long-running paginated extracts

### Conformance Checklist

- Implements lifecycle: initialize, connect, disconnect, isConnected
- Provides request primitives; read/download helpers only. Upload/write helpers are Full profile only and out of scope for this release
