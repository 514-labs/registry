...
**CRITICAL**: All connectors MUST follow the exact registry structure. This structure is enforced and essential for connector discovery, organization, and tooling.

#### Directory Structure
```
registry/
  {connector-name}/              # Connector root (kebab-case, e.g., "google-analytics")
    _meta/                       # Root connector metadata
      connector.json             # Root connector definition with schema
      README.md                  # Connector overview and documentation
      assets/                    # Shared assets (logos, diagrams)
    {author}/                    # Author/organization (e.g., "fiveonefour")
      _meta/                     # Author-specific metadata
        connector.json           # Author's implementation metadata
        README.md                # Author's implementation docs
        CHANGELOG.md             # Version history
        LICENSE                  # License file
        assets/                  # Author-specific assets
      {language}/                # Language implementation (e.g., "typescript")
        ...
        schemas/                 # Data schemas
          ...
          raw/                   # Raw API schemas
          extracted/             # Processed schemas
        tests/                   # Test files
          client.test.ts         # Unit tests
        package.json             # Language package definition
        tsconfig.json            # Language-specific config
```

#### Metadata Files

**Root connector.json** (`registry/{connector}/_meta/connector.json`):
```json
{
  ...
  "version": "1.0.0",
  "category": "api|database|file|stream",
  "tags": ["tag1", "tag2"],
  "description": "Brief description",
  "homepage": "https://api-provider.com"
}
```

**Author connector.json** (`registry/{connector}/{author}/_meta/connector.json`):
```json
{
  "name": "connector-name",
  "author": "author-name",
...
**Root `_meta/` directory** (`registry/{connector}/_meta/`):
- `connector.json` - Root connector definition (REQUIRED)
- `README.md` - Connector overview, provider info, available implementations
- `assets/` - Shared logos, diagrams, icons for the connector

**Author `_meta/` directory** (`registry/{connector}/{author}/_meta/`):
- `connector.json` - Author's implementation metadata (REQUIRED)  
- `README.md` - Author's implementation documentation
- `CHANGELOG.md` - Version history and release notes
- `LICENSE` - License file (MIT, Apache 2.0, etc.)
- `assets/` - Author-specific assets and branding

#### Structure Rules

1. **Connector names**: Use kebab-case, descriptive names (e.g., `google-analytics`, `stripe-payments`)
2. **Author names**: Use lowercase, no special characters (e.g., `fiveonefour`, `acmecorp`) 
3. **Language names**: Use lowercase language names (e.g., `typescript`, `python`, `go`)
4. **Metadata required**: Both root and author `_meta/` directories with `connector.json` files
5. **Source organization**: Language implementations must follow the `src/`, `docs/`, `examples/`, `schemas/`, `tests/` structure
6. **Schema compliance**: All `connector.json` files must validate against their respective schemas
7. **Documentation required**: All `_meta/` directories must contain `README.md` files
8. **Licensing required**: Author `_meta/` directories must contain `LICENSE` file

**WARNING**: Deviating from this structure will prevent connector registration and discovery. This structure enables automated tooling for generation, testing, documentation, and publishing.
