...
export type OpenWeatherConfig = {
  ...
  retry?: CoreConfig['retry']
  rateLimit?: CoreConfig['rateLimit']
  timeoutMs?: number
}

export class Connector extends ApiConnectorBase {
  init(userConfig: OpenWeatherConfig): this {
    ...
    const coreConfig: CoreConfig = {
      ...
      auth: {
        type: 'bearer',
        bearer: { token: '' } // Dummy auth; API key goes in query params
      },
      defaultQueryParams: {
        appid: userConfig.apiKey
      },
    ...
    const cfg = (this as any).config as CoreConfig
