...
it('logging includes url, headers, body when enabled', async () => {
  ...
  nock(BASE).get('/products').query({ x: '1' }).reply(200, [{ id: 1 }], { 'x-test-header': 'ok' })
  ...
  await (conn as any).request({ method: 'GET', path: '/products?x=1' })
  ...
  expect(String(respEvt.url)).toContain('/products?x=1')
