...
import { createResource as createAircraftResource } from '../resources/aircraft'

export type AdsBlolConnectorConfig = {
  baseUrl?: string
  logging?: { 
    enabled?: boolean
...
export class Connector extends ApiConnectorBase {
  init(userConfig?: AdsBlolConnectorConfig) {
    const baseUrl = userConfig?.baseUrl ?? 'https://api.adsb.lol'
    
    const coreConfig: CoreConfig = {
      baseUrl,
      userAgent: 'ads-b-dot-lol-connector',
      auth: { type: 'bearer', bearer: { token: '' } }, // No auth needed, but required by core
    }
  ...
  get aircraft() { return createAircraftResource(this.sendLite as any) }
}
