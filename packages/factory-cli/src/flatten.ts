  ...
  for (const [key, def] of Object.entries(props)) {
    ...
    if ((base as any).type === 'array' && (base as any).items && typeof (base as any).items === 'object') {
      ...
      if (isObjectSchema(items)) {
        ...
        // If property is optional and allows null, strip null from the type
        // If required and allows null, preserve nullability
        out.properties[arrName] = (isReq || !nullable)
          ? arraySchema
...
export async function runFlatten(opts: FlattenOpts) {
  ...
  const typeName = name
  // Remove existing exported interface/type with the same name
  const pattern = new RegExp(
    String.raw`^export\s+(?:interface|type)\s+${typeName}\b[\s\S]*?(?=^export\s+(?:interface|type)\s+|\Z)`,
    'm'
  )
