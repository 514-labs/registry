...
async function findRepoRoot(startDir: string): Promise<string> {
  ...
  while (true) {
    ...
    const hasRegistries = await pathExists(
      path.join(dir, "connector-registry")
    );
...
function assertPattern(
  ...
  if (!re.test(value))
    throw new Error(`${label} does not match pattern ${pattern}: ${value}`);
  return value;
}
...
function getScaffoldPath(
  ...
  const registryDir =
    kind === "connector" ? "connector-registry" : "pipeline-registry";
...
export async function runScaffold({
  ...
  const ask = async () => {
    ...
    pushQ(
      "version",
      "Version (e.g. v1, ga4, 2024-10-01)",
      options.scaffoldVersion ?? options.version
    );
    pushQ("author", "Author org/user (kebab-case)", options.author);

    pushQ(
      "implementation",
      `Implementation (default: ${varsSpec.implementation?.default ?? "default"})`,
      options.implementation ?? varsSpec.implementation?.default ?? "default"
    );
    pushQ("packageName", "Package name (npm or python)", options.packageName);
    pushQ(
      "resource",
      `Default resource (kebab-case)`,
    ...
    const answers = options.yes
      ? {
          ...
          implementation:
            ...
            varsSpec.implementation?.default ??
            "default",
  ...
  assertPattern(
    answers.version,
    varsSpec.version?.pattern ?? "^[A-Za-z0-9][A-Za-z0-9._-]*$",
    "version"
  );
  assertPattern(
    answers.author,
    varsSpec.author?.pattern ?? "^[a-z0-9](?:[a-z0-9-]{0,37}[a-z0-9])?$",
    "author"
  );
  assertPattern(
    answers.implementation,
    varsSpec.implementation?.pattern ?? "^[a-z0-9][a-z0-9-]*$",
    "implementation"
  );
  if (!answers.packageName)
    answers.packageName = suggestPackageName(kind, language, id);
  if (language === "typescript") {
    const npmPattern =
      varsSpec.packageName?.pattern ?? "^[a-z0-9-~][a-z0-9-._~]*$";
  ...
  assertPattern(
    answers.resource,
    varsSpec.resource?.pattern ?? "^[a-z0-9][a-z0-9-]*$",
    "resource"
  );
  ...
  const headline = options.dryRun
    ? kleur.yellow("Dry run: no files written")
    : kleur.green("Scaffold created");
