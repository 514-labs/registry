...
function deriveFromSpec(spec) {
  ...
  nodes.push({
    id: sourceId,
    kind: "source",
    label: spec?.source?.connector?.name || spec?.source?.type || "source",
  });
  const systemIds = [];
...
async function main() {
  ...
  const lineage =
    spec.lineage && spec.lineage.nodes?.length
      ? spec.lineage
      : deriveFromSpec(spec);
  ...
  console.log(
    "Wrote",
    mmdPath,
    process.argv.includes("--svg") ? "and SVG" : ""
  );
}
